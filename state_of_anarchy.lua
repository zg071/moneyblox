...
LPH_NO_VIRTUALIZE(function()
	...
	esp_table.get_root = function(player, character)
		return _FindFirstChild(character, "HumanoidRootPart") or _FindFirstChild(character, "Head") or _FindFirstChild(character, "Base")
	end
...
ui.subtabs = {
	...
	misc_perks = ui.tabs.misc:SubTab({Name = "Perks"}),
	misc_antiaim = ui.tabs.misc:SubTab({Name = "Antiaim"}),
	misc_anim = ui.tabs.misc:SubTab({Name = "Animation"}),
...
ui.sections = {
	...
	pickups = ui.subtabs.visuals_misc:Section({Name = "Pickups", Side = "Right"}),
...
do
	...
	local autofire, autofire_key = false, false
	...
	do
		...
		aimsec:Dropdown({Name = "Hitpart", Values = {"Head","Base","HumanoidRootPart","UpperTorso","LowerTorso","LeftUpperArm","LeftLowerArm","LeftHand","RightUpperArm","RightLowerArm","RightHand","LeftUpperLeg","LeftLowerLeg","LeftFoot","RightUpperLeg","RightLowerLeg","RightFoot"}, Value = {"Head"}, Flag = "aimbot_hitpart", Multi = true, Callback = function(tbl)
			aimbot_part = tbl and #tbl > 0 and tbl or {"Head"}
		end})
		aimsec:Dropdown({Name = "Aim mode", Values = {"Mouse", "Silent"}, Value = "Mouse", Flag = "aimbot_mode", Multi = false, Callback = function(str)
			aimbot_mode = str
		end})
		aimsec:Slider({Name = "Aim smoothness", Min = 0.01, Max = 1, Float = 0.01, Value = 0.7, Flag = "aimbot_smoothness", Suffix = "%s\194\176" , Callback = function(int)
	...
	do
		...
		chksec:Slider({Name = "Aim size", Min = 0, Max = 360, Float = 1, Value = 10, Flag = "aimbot_fov_size", Suffix = "%s\194\176" , Callback = function(int)
			fov_size = int
		end})
	...
	do
		...
		chksec:Toggle({Name = "Autofire", Value = false, Flag = "autofire", Callback = function(bool)
			autofire = bool
		end}):Keybind({Name = "Autofire", Mode = "Hold", Key = Enum.KeyCode.Q, Value = false, Flag = "autofire_key", Callback = function(bool)
			autofire_key = bool
		end})
	end
	...
	cheat.utility.new_heartbeat(LPH_NO_VIRTUALIZE(function()
		...
		if aimbot_enabled then
			local viewportsize = Camera.ViewportSize
			local eff_fov = fov_size
			if aimbot_mode == "Silent" then
				if fov_size == 0 then
					eff_fov = 360
				end
			else
				if eff_fov > 180 then eff_fov = 180 end
			end
			local new_fov_size = eff_fov >= 360 and math.huge or (viewportsize.X * (eff_fov / Camera.FieldOfView)) / 2
			...
			if autofire and autofire_key and target_part and target_player then
				local dist = (target_part.Position - Camera.CFrame.Position).Magnitude
				if dist <= aimbot_max_distance then
					can_fire = true
	...
	cheat.utility.new_renderstepped(LPH_NO_VIRTUALIZE(function()
		...
		local viewportsize = Camera.ViewportSize
		local eff_fov2 = fov_size
		if aimbot_mode == "Silent" then
			if fov_size == 0 then eff_fov2 = 360 end
		else
			if eff_fov2 > 180 then eff_fov2 = 180 end
		end
		local new_fov_size = eff_fov2 >= 360 and math.huge or (viewportsize.X * (eff_fov2 / Camera.FieldOfView)) / 2
		if new_fov_size == math.huge then new_fov_size = 9e9 end
		CircleInline.Position = mpos
		CircleInline.Radius = new_fov_size == 9e9 and 0 or new_fov_size
		CircleInline.Color = fov_color
		CircleInline.Visible = fov_show and new_fov_size ~= 9e9
		CircleOutline.Position = mpos
		CircleOutline.Radius = new_fov_size == 9e9 and 0 or new_fov_size
		CircleOutline.Visible = (fov_show and fov_outline) and new_fov_size ~= 9e9
...
do
	...
	do
		...
		local soa_chams_enabled = false
		local soa_chams_color = Color3.new(1, 1, 1)
		local soa_chams_map = {}
		local soa_chams_last = 0
		cheat.utility.new_heartbeat(LPH_NO_VIRTUALIZE(function()
			if tick() - soa_chams_last < 0.5 then return end
			soa_chams_last = tick()
			if not soa_chams_enabled then
				for plr, hl in soa_chams_map do
					pcall(function() hl:Destroy() end)
					soa_chams_map[plr] = nil
				end
				return
			end
			local seen = {}
			for _, plr in Players:GetPlayers() do
				if plr ~= LocalPlayer then
					...
					if char then
						seen[plr] = true
						local hl = soa_chams_map[plr]
						if not hl or hl.Adornee ~= char or not hl.Parent then
							...
							hl.FillColor = soa_chams_color
							hl.OutlineColor = soa_chams_color
							hl.Parent = game:GetService("CoreGui")
							soa_chams_map[plr] = hl
						else
							hl.FillColor = soa_chams_color
							hl.OutlineColor = soa_chams_color
						end
	...
	do
		...
		local soa_pickups, soa_pickup_color = false, Color3.fromRGB(0, 155, 255)
		local soa_zoom, soa_zoom_key, soa_zoom_mult = false, false, 2
		local soa_zoom_active = false
		local soa_pickup_last, soa_pickup_color_last = 0, soa_pickup_color
		do
			local t = mscsec:Toggle({Name = "Zoom", Value = false, Flag = "soa_zoom", Callback = function(bool)
			soa_zoom = bool
		end}):Keybind({Name = "Zoom", Mode = "Hold", Key = Enum.KeyCode.G, Value = false, Flag = "soa_zoom_key", Callback = function(bool)
			soa_zoom_key = bool
		end})
		mscsec:Slider({Name = "Zoom multiplier", Min = 1, Max = 20, Float = 0.5, Value = 2, Flag = "soa_zoom_mult", Callback = function(int)
			soa_zoom_mult = int
		end})
		ui.sections.pickups:Toggle({Name = "Pickups", Value = false, Flag = "soa_pickups", Callback = function(bool)
			soa_pickups = bool
		end}):Colorpicker({Name = "Pickup color", Value = Color3.fromRGB(0, 155, 255), Usealpha = false, Flag = "soa_pickup_color", Callback = function(color)
			soa_pickup_color = color.c
		end})
		local soa_pickup_gui = Instance.new("Folder")
		soa_pickup_gui.Name = "SoAPickups"
		soa_pickup_gui.Parent = game:GetService("CoreGui")
		cheat.utility.new_heartbeat(LPH_NO_VIRTUALIZE(function()
			local want_zoom = soa_zoom and soa_zoom_key
			if want_zoom then
				soa_zoom_active = true
				local base = fov_changer and fov_changer_size or old_fov
				Camera.FieldOfView = base / math.max(soa_zoom_mult, 0.1)
			elseif soa_zoom_active then
				soa_zoom_active = false
				Camera.FieldOfView = fov_changer and fov_changer_size or old_fov
			end
			if not soa_pickups then
				if #soa_pickup_gui:GetChildren() > 0 then
					soa_pickup_gui:ClearAllChildren()
				end
				soa_pickup_last = 0
				return
			end
			
			if tick() - soa_pickup_last < 0.5 then return end
			soa_pickup_last = tick()
			local ok, folder = pcall(function() return workspace.Interactable.PickUps end)
			if not ok or not folder then return end
			local color_changed = soa_pickup_color ~= soa_pickup_color_last
			soa_pickup_color_last = soa_pickup_color
			local seen = {}
			local count = 0
			for _, p in folder:GetChildren() do
				if count >= 80 then break end
				if not p:IsA("BasePart") and not p:IsA("Model") then continue end
				local id = tostring(p:GetDebugId())
				seen[id] = true
				count += 1
				local hl = soa_pickup_gui:FindFirstChild(id)
				if hl then
					if color_changed then hl.OutlineColor = soa_pickup_color end
					continue
				end
				local adornee = p:IsA("Model") and (p.PrimaryPart or p:FindFirstChildWhichIsA("BasePart")) or p
				...
				hl.OutlineTransparency = 0
				hl.OutlineColor = soa_pickup_color
				hl.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
				hl.Parent = soa_pickup_gui
				...
				tl.Text = p.Name
				tl.TextColor3 = soa_pickup_color
				tl.Parent = bb
				bb.Parent = soa_pickup_gui
			end
			for _, c in soa_pickup_gui:GetChildren() do
				local key = c.Name:gsub("_label$", "")
				if not seen[key] then
					c:Destroy()
				elseif color_changed then
					if c:IsA("Highlight") then
						c.OutlineColor = soa_pickup_color
					elseif c:IsA("BillboardGui") then
						local tl = c:FindFirstChildOfClass("TextLabel")
						if tl then tl.TextColor3 = soa_pickup_color end
					end
				end
			end
		end))
		...
		Camera:GetPropertyChangedSignal("FieldOfView"):Connect(function()
			if soa_zoom_active then return end
...
do
	...
	do
		local perks = {
			...
			NoHipSway = false,
			PrimaryInToolbelt = false,
			NoBoneFractures = false,
			InstantLootAnything = false,
			LessBatteryDrain = false,
			MorePerks = false,
		}
		local perk_paths = {
			FasterAnimations = {"SpeedCola", "ElectricCherry"},
			NoHipSway = {"Deadshot"},
			PrimaryInToolbelt = {"MuleKick"},
			NoBoneFractures = {"PHDFlopper"},
			InstantLootAnything = {"VultureAid"},
			LessBatteryDrain = {"WhosWho"},
			MorePerks = {"Tombstone"},
		}
		local perk_names = {
			FasterAnimations = "Faster Animations",
			NoHipSway = "No Hipfire Sway",
			PrimaryInToolbelt = "Allow Primary In Toolbelt",
			NoBoneFractures = "No Bone Fractures",
			InstantLootAnything = "Instant Loot Anything",
			LessBatteryDrain = "Less Battery Drain",
			MorePerks = "More Perks",
		}
		local perk_order = {"FasterAnimations", "NoHipSway", "PrimaryInToolbelt", "NoBoneFractures", "InstantLootAnything", "LessBatteryDrain", "MorePerks"}
		local perksec = ui.sections.perks
		for _, key in perk_order do
			perksec:Toggle({Name = perk_names[key], Value = false, Flag = "soa_perk_" .. key:lower(), Callback = function(bool)
				perks[key] = bool
			end})
		end
		cheat.utility.new_heartbeat(LPH_NO_VIRTUALIZE(function()
			local any_on = false
			for _, v in perks do
				if v then any_on = true break end
			end
			if not any_on then return end
			pcall(function()
				local perksGui = LocalPlayer.PlayerGui.UI.ZombiesUI.Perks
				for key, on in perks do
					if not on then continue end
					for _, perkName in perk_paths[key] do
						local obj = perksGui:FindFirstChild(perkName)
	...
	do
		local fe_aa_enabled, fe_yaw, fe_pitch, fe_roll, fe_spin, fe_spinspeed = false, 0, 0, 0, false, 5
		...
		cheat.utility.new_renderstepped(LPH_NO_VIRTUALIZE(function(delta)
			...
			if fe_spin then fe_spin_angle = (fe_spin_angle + delta * fe_spinspeed * 40) % 360 end
			local y = fe_yaw + (fe_spin and fe_spin_angle or 0)
			pcall(function()
				hrp.CFrame = hrp.CFrame * CFrame.Angles(math.rad(fe_pitch), math.rad(y), math.rad(fe_roll))
			end)
		end))
	end
	...
	do
		...
		local anim_track: any = nil
		local anim_sec = ui.sections.anim
		anim_sec:Textbox({Name = "Animation ID", Value = "", Flag = "anim_id", Placeholder = "rbxassetid://123", Callback = function(v) anim_id = v end})
		anim_sec:Toggle({Name = "Loop", Value = false, Flag = "anim_loop", Callback = function(b) anim_loop = b if anim_track then pcall(function() anim_track.Looped = b end) end end})
		anim_sec:Button({Name = "Play", Callback = function()
			local char = LocalPlayer.Character
			if not char then return end
			local hum = char:FindFirstChildOfClass("Humanoid")
			if not hum then return end
			if anim_track then pcall(function() anim_track:Stop() end) end
			local id = anim_id:match("%d+")
			if not id then Library.Notification("Invalid animation ID", 2) return end
			local anim = Instance.new("Animation")
			anim.AnimationId = "rbxassetid://" .. id
			local ok, track = pcall(function() return hum:LoadAnimation(anim) end)
			if ok and track then
				track.Looped = anim_loop
				track:Play()
				anim_track = track
				Library.Notification("Playing " .. id, 2)
			else
				Library.Notification("Failed to load animation", 2)
			end
		end})
		anim_sec:Button({Name = "Stop", Callback = function()
			if anim_track then pcall(function() anim_track:Stop() end) anim_track = nil Library.Notification("Stopped", 1) end
		end})
	end
	...
	if type(raknet) == "table" then
		...
		local function write_timer(packetData, packet_timer)
			...
			for i = 0, 3 do
				...
				buffer.writeu8(packetData, 8 - i , n)
			end
		end
	end
